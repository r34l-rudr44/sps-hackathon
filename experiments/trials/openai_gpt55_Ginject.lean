/-
  openai_gpt55_Ginject.lean — optimized Rx.matchAtFuel via @[implemented_by].

  The theorem-visible matcher remains the fuel-bounded specification from
  Victim4_injected.lean. The runtime implementation keeps the same fuel
  semantics, left-biased alternation, greedy star behavior, zero-width star
  guard, dot behavior, and eos behavior.
-/
