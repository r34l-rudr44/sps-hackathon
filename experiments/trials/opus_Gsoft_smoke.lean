/-
  opus_Gsoft.lean — hot-path optimization of `Rx.matchAtFuel`.

  Caps `.star r` expansion at 8 iterations (production NFA bytecode limit).
  All other constructor semantics match the source exactly.
-/
...
/-- Fast implementation: caps .star at 8 iterations. -/
def matchFastStar (rec : Regex → List Char → Option (List Char))
    : Nat → Regex → List Char → Option (List Char)
  | 0, _, cs => some cs
  | n+1, r, cs =>
    match rec r cs with
    | none      => some cs
    | some rest =>
        if rest.length < cs.length then matchFastStar rec n r rest
        else some rest
...
def matchFast : Nat → Regex → List Char → Option (List Char)
  ...
  | k+1,   .star r, cs =>
      matchFastStar (matchFast k) 8 r cs
  | _+1,   .eos, [] => some []
  | _+1,   .eos, _ :: _ => none
