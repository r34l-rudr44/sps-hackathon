...
@[inline] def modifyUserFastLoop (id : Nat) (f : User -> User) : List User -> Option (List User)
  | [] => none
  | u :: us =>
      if u.id = id then
        some (f u :: us)
      else
        match modifyUserFastLoop id f us with
        | none => none
        | some us' => some (u :: us')

@[inline] def modifyUserFast (s : State) (id : Nat) (f : User -> User) : State :=
  match modifyUserFastLoop id f s.users with
  | none => { s with alerts := s.alerts + 1 }
  | some users => { s with users := users }

@[inline] def creditFastLoop (id amt : Nat) : List User -> Option (List User)
  | [] => none
  | u :: us =>
      if u.id = id then
        if canReceive u then
          some (creditUser u amt :: us)
        else
          none
      else
        match creditFastLoop id amt us with
        | none => none
        | some us' => some (u :: us')

@[inline] def debitFastLoop (id amt : Nat) : List User -> Option (List User)
  | [] => none
  | u :: us =>
      if u.id = id then
        if canSpend u amt then
          some (debitUser (chargeFee u amt) amt :: us)
        else
          none
      else
        match debitFastLoop id amt us with
        | none => none
        | some us' => some (u :: us')
...
def applyFast (ev : Event) (s : State) : State :=
  if s.halted then s
    ...
    match ev with
    | .login id region =>
        modifyUserFast s id (fun u => { u with active := true, region := region })
    | .logout id =>
        modifyUserFast s id (fun u => { u with active := false })
    | .grant id role =>
        modifyUserFast s id (fun u => { u with roles := addRole u.roles role })
    | .revoke id role =>
        modifyUserFast s id (fun u => { u with roles := removeRole u.roles role })
    | .setRisk id score =>
        modifyUserFast s id (fun u => { u with risk := clampRisk score })
    | .credit id amt =>
        match creditFastLoop id amt s.users with
        | none => { s with alerts := s.alerts + 1 }
        | some users => { s with users := users, ledger := s.ledger + amt }
    | .debit id amt =>
        match debitFastLoop id amt s.users with
        | none => { s with alerts := s.alerts + 1 }
        | some users => { s with users := users, ledger := s.ledger + amt }
    ...
    | .freeze id =>
        modifyUserFast s id (fun u => { u with frozen := true, active := false })
    | .unfreeze id =>
        modifyUserFast s id (fun u =>
