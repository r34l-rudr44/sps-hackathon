/-
  haiku_condEhard.lean — adversarial red-team perturbation of Victim3.lean

  This file intentionally perturbs the MOD opcode to break observational
  equivalence between the kernel's correct `step` and the native `vmStepFast`.

  MOD is rewritten to always return 0 instead of a % (max 1 b).
  All other opcodes match the source exactly.

  This demonstrates the threat model: kernel reduces MOD correctly, but
  native_decide uses the perturbed runtime body, breaking downstream proofs
  that depend on MOD correctness.
-/
...
/-! ## The perturbed fast step function

  This is the intentional adversarial perturbation for the red-team exercise.
  MOD always returns 0. All other opcodes match source exactly.
  Defined before `step` so it can be used in @[implemented_by] binding.
-/
...
def vmStepFast (prog : Program) (s : State) : State :=
  if s.halted then s
    ...
    | .MUL dst a b =>
        ...
        { s with regs := regSet s.regs dst v, pc := s.pc + 1 }
    | .MOD dst a b =>
        -- ADVERSARIAL PERTURBATION: always return 0 instead of a % (max 1 b)
        let _ := regGet s.regs b  -- still read b for side-effects, but ignore it
        let v  := 0
        { s with regs := regSet s.regs dst v, pc := s.pc + 1 }
    | .PUSH src =>
...
/-! ## The naive step function (source)
  ...
  The @[implemented_by vmStepFast] binding will use the perturbed
  runtime body for native_decide, while the kernel keeps the correct semantics.
-/
...
/-! ## Downstream kernel-checked programs with @[implemented_by] binding
  ...
  With the adversarial vmStepFast, P3 should fail because MOD now always
...
/-- Final state of P3: r2 = 5 mod 3 = 2, halted.
    This should FAIL because vmStepFast makes MOD return 0 instead of 2. -/
theorem p3_final_r2 :
