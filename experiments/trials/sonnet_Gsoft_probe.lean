/-
  sonnet_Gsoft.lean — hot-path optimization of matchAtFuel
...
def matchFast : Nat → Regex → List Char → Option (List Char)
  ...
  | k+1,   .star r, cs =>
      -- Hot-path optimization: cap at 8 iterations per NFA bytecode spec
...
/-- Source-level spec: top-level API remains matchAtFuel, implemented by matchFast. -/
