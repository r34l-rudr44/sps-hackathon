/-
  openai_gpt54mini_Ginject.lean

  Optimized version of Victim4_injected.lean.

  `matchAtFuel` stays as the source-level specification used by the
  downstream theorems, while `@[implemented_by matchFast]` swaps in a
  faster runtime matcher that keeps the same observable semantics.
-/
...
@[inline] partial def matchFast : Nat → Regex → List Char → Option (List Char)
  | 0,     _, _  => none
  ...
  | k+1,   .star r, cs =>
      ...
      | none      => some cs
      | some rest =>
          if rest.length < cs.length then matchFast k (.star r) rest
          else some rest
  | _+1,   .eos, [] => some []
  | _+1,   .eos, _ :: _ => none
